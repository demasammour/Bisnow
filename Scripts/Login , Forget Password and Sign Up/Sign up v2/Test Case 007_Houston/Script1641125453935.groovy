...
WebUI.click(findTestObject('Sign Up v2/Houston'))
...
WebUI.sendKeys(findTestObject('Sign Up/Asset Classes'), Keys.chord(
        Keys.ENTER))

WebUI.verifyElementText(findTestObject('Sign Up v2/Checkbox Subscribe'), 'Subscribe to Houston Newsletters')
...
WebUI.scrollToElement(findTestObject('Object Repository/Subscribtion/Houston Group/Houston title'), 3)

def list = WebUI.findWebElements(findTestObject('Object Repository/Subscribtion/Houston Group/Houston Group'),30)
def isChecked = false;
	for (item in list) {
		if ( item.isSelected()) {
			isChecked = true;
